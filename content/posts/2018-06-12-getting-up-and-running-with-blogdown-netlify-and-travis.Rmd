...
seems as most other people in the community just commit html or md files, but 
I figured that I'd like to deploy my page without committing these derivatives
and operate on pure Rmarkdown instead. A quick Google search did not 
...
the hugo version as part of the build command (like `hugo_0.19`) or via an 
...
to see where the site got deployed to. Copy this URL in a browser and check out 
if the page looks as expected. Keep in mind that the netlify CI check that is 
shown in the GitHub PR is the empty page, and you should really check the 
netlify page build from travis.
I nevertheless changed the settings in netlify for the deployment for the 
former so I get three green ticks.

For the `build.sh` to get called, you obviously also need to change your
`.traivs.yml`. Here is my final version:
...
Overall, I am pretty happy with the set-up now. 
Looking back, it would have been much easier to build the page locally and 
simply commit md or html derivatives. 
If I'd forseen the trouble involved in getting
it working, I possibly would have choosen this approach. Now that I figured out
how you can do without committing derivaties, I hope some people find this post 
useful and decide to adapt this strategy. In case you are interested in some of 
the details I skipped above for the sake of brievity, feel free to reach out to
me via the comment functionality below. I hope these instructions are 
informative so that you'll need a fraction of the time I spend on configuring 
my site.
